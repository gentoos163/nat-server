...
8. [命令行参考](#8-命令行参考)
   ...
   - [GUI 模式](#82-gui--启动桌面界面)
   - [基础查询](#83-基础查询命令)
   - [用户与设备管理](#84-用户与设备管理命令)
   - [隧道连接](#85-隧道连接命令)
   - [端口转发规则管理](#86-端口转发规则管理命令)
   - [调试](#87-调试命令)
   - [内部命令](#88-内部命令)
...
| **桌面 GUI** | Apple 液态玻璃风格 Slint 界面，深/浅色主题，系统托盘 |
| **订阅管理** | 支持支付宝二维码 + Stripe 在线支付，GUI 内完成订阅 |
| **自动更新** | 启动后静默检查，一键下载安装（参考 RustDesk 实现） |
...
│                                                                      │
│  ┌──────────────┐  ┌─────────────────────┐  ┌───────────────────┐   │
│  │  Slint GUI   │  │  RendezvousMediator │  │   LAN Discovery   │   │
│  │  + 系统托盘   │  │  (TCP → hbbs:21116) │  │ (UDP 广播 :21119) │   │
│  └──────┬───────┘  │  ┌──────────────┐   │  └───────────────────┘   │
│         │ IPC      │  │ user_token ◄─┼───┼───────────────────────── │
│  ┌──────▼───────┐  │  │ (JWT注入)    │   │        JWT 存储           │
│  │  Auth Module │  │  └──────────────┘   │                          │
...
│                    │  (直连 / 中继隧道)   │  │ (127.0.0.1:21114)   │ │
│                    └─────────────────────┘  └─────────────────────┘ │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────────┐ │
│  │  Updater（后台线程）                                             │ │
│  │  启动 30 秒后首检 → 之后每 24 小时 → 发现新版本时通知 GUI        │ │
│  └─────────────────────────────────────────────────────────────────┘ │
└──────────────┬───────────────────────────────────────────────────────┘
               │ NAT
 ══════════════╪══════════════════════════════════════════════════════════
...
### `ui/app.rs` — Slint GUI 桥接层

负责 Slint UI 与守护进程之间的状态同步与事件处理。

| 职责 | 说明 |
|---|---|
| 回调绑定 | 将 Slint 按钮/输入事件绑定到 IPC 命令或本地函数 |
| 状态轮询 | 后台定时拉取守护进程状态，更新 UI 数据模型 |
| Logo 注入 | 启动时将 `logo.ico` 嵌入为窗口图标和侧边栏 Logo |
| 更新触发 | 将 `updater.rs` 的版本检查结果推送到 UI，触发更新提示 |
| 页面路由 | 管理 8 个功能页面之间的切换逻辑 |

---

### `ui/tray.rs` — 系统托盘

基于 `tray-icon 0.24` 实现跨平台系统托盘。

| 特性 | 说明 |
|---|---|
| 左键 | 显示/隐藏主窗口 |
| 右键菜单 | 显示窗口、退出等常用操作 |
| 在线状态指示 | 托盘图标右下角绿色（在线）/ 灰色（离线）圆点覆盖 |
| 图标 | 使用 `icons/logo.ico`（编译时嵌入） |

---

### `updater.rs` — 自动更新模块

参考 RustDesk 更新机制实现，全平台统一接口。

**关键设计**：

| 特性 | 说明 |
|---|---|
| 全局 channel | `TX_MSG` 控制后台检查线程，支持手动触发和自动定时 |
| 首次检查 | 启动 30 秒后自动执行，避免与启动流程竞争 |
| 定时检查 | 之后每 24 小时一次 |
| 手动检查 | GUI"检查更新"按钮触发，最小间隔 10 分钟（防频繁请求） |
| 增量下载 | HEAD 请求预比对文件大小，已下载则跳过 |
| SHA256 校验 | 下载完成后自动校验（服务端配置了校验值时启用） |
| 进度回调 | 流式下载，实时上报进度到 GUI 进度条 |
...
# 默认构建（含 GUI，Slint + 系统托盘）
cargo build -p nat-client --release

# 无 GUI 构建（仅 daemon + CLI，适合服务器/无头环境）
cargo build -p nat-client --release --no-default-features
...
| Feature | 默认 | 说明 |
|---|---|---|
| `gui` | 启用 | Slint 桌面窗口 + 系统托盘 + 自动更新 UI |
...
| 选项 | 必须 | 默认值 | 说明 |
|---|---|---|---|
| `-s, --server <ADDR>` | 是 | — | hbbs 服务器地址（`host` 或 `host:port`） |
| `-r, --relay <ADDR>` | 否 | server+1端口 | hbbr 中继服务器地址 |
| `--id <ID>` | 否 | 自动生成 | 指定本机 Peer ID（9 位数字） |
| `--ipc-port <PORT>` | 否 | 21114 | IPC 控制接口端口 |
| `--log-level <LEVEL>` | 否 | info | 日志级别：`trace/debug/info/warn/error` |
...
```bash
nat-client gui --server <ADDR> [选项]
```

启动 Slint 桌面窗口，同时在后台启动守护进程和系统托盘。

| 选项 | 必须 | 默认值 | 说明 |
|---|---|---|---|
| `-s, --server <ADDR>` | 是 | — | hbbs 服务器地址 |
...
| 页面 | 说明 |
|---|---|
| 首页 | 连接状态概览、Peer ID 展示 |
| 连接 | 发起/管理点对点连接 |
| 局域网 | 发现并连接局域网内的节点 |
| 账户 | 登录/注册/查看用户信息 |
| 设备 | 查看账户下绑定的设备列表 |
| 设置 | 服务器地址、日志级别、"检查更新"按钮 |
| 隧道接入 | 转发规则管理、服务扫描 |
| 订阅套餐 | 查看套餐、发起支付 |
...
- 左键：显示/隐藏主窗口
- 右键：弹出菜单（显示窗口、退出）
- 图标：`icons/logo.ico` 编译时嵌入，右下角显示在线（绿色）/ 离线（灰色）状态圆点

> **注意**：GUI 功能由 Cargo feature `gui` 控制。使用 `--no-default-features` 构建时不包含此子命令。
...
| `-u, --username` | 是 | 用户名 |
| `-e, --email` | 是 | 邮箱地址 |
| `-p, --password` | 是 | 密码（最少 6 位） |
| `--device-name` | 否 | 设备名称，默认为 `nat-client@hostname` |
...
> 移除的是目标设备对应的**绑定关系**，不影响对方守护进程运行，但该设备将无法再通过用户身份注册。
...
| 选项 | 必须 | 默认值 | 说明 |
|---|---|---|---|
| `-p, --peer-id <ID>` | 是 | — | 目标 Peer ID |
| `-l, --local-port <PORT>` | 否 | 0（自动分配） | 本地监听端口 |
...
| 选项 | 必须 | 默认值 | 说明 |
|---|---|---|---|
| `-n, --name <名称>` | 是 | — | 规则名称（如 `SSH`） |
| `-t, --target-port <PORT>` | 是 | — | 本地服务端口 |
| `--target-host <IP>` | 否 | `127.0.0.1` | 目标主机（支持内网其他机器） |
| `--peer-id-filter <ID>` | 否 | `""` | 限定特定对端（空 = 任意对端） |
...
以下命令由更新流程自动调用，用户通常不需要直接使用。

#### `apply-update` — 应用更新（Windows 专用）

```bash
nat-client apply-update --old-exe <PATH>
```

Windows 更新流程：新版本进程以此参数启动，等待旧进程退出后覆盖可执行文件，然后重启。详见 [自动更新模块说明](#updaters--自动更新模块)。
...
| `discover` | `{"cmd":"discover"}` | `{"peers":[...]}` （约3秒） |
...
1. 页面从服务端 `/api/subscription/plans` 拉取可用套餐列表
2. **支付宝**：点击套餐后客户端内展示二维码，每 3 秒轮询支付结果
3. **Stripe**：点击套餐后打开系统浏览器跳转 Checkout 页面，客户端同步轮询结果
4. 支付成功后自动刷新订阅状态显示

---

### 场景七：作为系统服务（Linux systemd）
...
└── nat-client/                  ← 客户端代码（本工具）
    ├── config.example.toml      ← 配置文件示例（含所有字段的中文注释）
    └── src/
        ├── main.rs              ← CLI 入口（含 apply-update 隐藏命令）
        ├── auth.rs              ← 用户认证与设备管理
        ├── config.rs            ← 配置管理（ForwardRule + 认证字段）
        ├── rendezvous_mediator.rs ← 连接 hbbs（RegisterPk 注入 user_token）
        ├── ipc.rs               ← 本地控制接口
        ├── lan.rs               ← LAN 发现
        ├── port_forward.rs      ← TCP 隧道 + 本机服务扫描
        ├── updater.rs           ← 自动更新
        └── ui/
            ├── app.rs           ← Slint GUI 桥接层
            └── tray.rs          ← 系统托盘
```
...
1. 检查服务端是否配置了 `client_latest_version` 和对应平台的下载链接
2. 确认下载链接可公网访问
3. 如配置了 SHA256，确认校验值与实际文件匹配
4. 可在 GUI 设置页面手动点击"检查更新"重试（最小间隔 10 分钟）
5. 查看守护进程日志中 `[updater]` 前缀的条目获取详细错误信息
...
> **重置后会生成新的 Peer ID 和密钥**，原有的连接关系和设备绑定需重新建立。
...
nat-client/
├── icons/
│   └── logo.ico                    # 应用图标（编译时嵌入到二进制）
│
├── ui/
│   └── main.slint                  # Slint UI 定义（8 页面，Apple 液态玻璃风格）
│
├── src/
│   ├── main.rs                     # CLI 入口（子命令解析 + apply-update 隐藏命令）
│   │
│   ├── ui/
│   │   ├── mod.rs                  # UI 模块声明
│   │   ├── app.rs                  # Slint GUI 桥接层（回调绑定、状态轮询、Logo 注入）
│   │   └── tray.rs                 # 系统托盘（tray-icon 0.24，在线/离线状态圆点覆盖）
│   │
│   ├── updater.rs                  # 自动更新（版本检查、流式下载、SHA256 校验、平台自替换）
│   ├── auth.rs                     # 用户认证与设备管理（HTTP REST API 封装）
│   ├── config.rs                   # 配置管理（Peer ID、密钥、JWT、转发规则持久化）
│   ├── ipc.rs                      # 本地 IPC 控制接口（127.0.0.1:21114，JSON-over-TCP）
│   ├── lan.rs                      # 局域网节点发现（UDP 广播 :21119）
│   ├── port_forward.rs             # TCP 隧道管理器（直连 + 中继 + 服务扫描）
│   ├── rendezvous_mediator.rs      # NAT 打洞中介（连接 hbbs，RegisterPk JWT 注入）
│   ├── socks5.rs                   # SOCKS5 代理支持
│   └── i18n.rs                     # 国际化
│
├── config.example.toml             # 配置文件示例（含所有字段的中文注释）
└── Cargo.toml                      # 包配置（features: gui）
```
...
*文档版本：v0.4.0 | 最后更新：2026 年（新增 GUI 模式、自动更新、订阅支付功能）*
