...
我不完全确定你的意图，先说说我目前理解的几种可能，你来确认：

1. **连接其他注册用户**（通讯录 / 在线用户列表）  
   目前 UI 只有"手动输入 Peer ID"才能连接，没有从服务端拉取"所有在线用户"然后点选的入口。类似 RustDesk 的地址本功能。

2. **端口转发规则管理的可视化入口**  
   后端已有 `list-rules / add-rule / remove-rule`，但 UI 里完全没有这个页面，用户无法在 GUI 里管理"哪些本地服务对外暴露"。

3. **SOCKS5 代理出口节点选择**  
   设置页里有出口 Peer ID 的文本框，但没有"从在线节点里选一个"的下拉/弹窗。

4. **接入第三方应用（让其他 App 走 NAT 隧道）**  
   比如让 SSH 客户端、浏览器通过 SOCKS5 代理走隧道，需要一个 UI 来配置哪个应用用哪条隧道。

---

你说的"第三方用户/应用"具体是哪种场景？或者完整描述一下你设想的交互流程，我再来做。
